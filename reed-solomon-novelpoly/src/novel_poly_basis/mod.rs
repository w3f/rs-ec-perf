...
pub fn reconstruct<'a, S: Shard<f2e16::Additive>>(
    ...
    let rs = ReedSolomon::<f2e16::Additive>::new(
        validator_count,
    ...
    [u8; F::FIELD_BYTES]: Sized,
{
        ...
        for (chunk_idx, i) in (0..bytes.len()).into_iter().step_by(k2).enumerate() {
            ...
            for val_idx in 0..validator_count {
                shards[val_idx].set_chunk(
                    chunk_idx,
                    AsRef::<[u8; <Additive as FieldAdd>::FIELD_BYTES]>::as_ref(
                        &encoding_run[val_idx],
                    ),
                );
            }
        }
    ...
    pub fn reconstruct<S: Shard<F>>(&self, received_shards: Vec<Option<S>>) -> Result<Vec<u8>> {
        ...
        let received_shards = <ShardHold<S, F>>::equalize_shards_number_with_code_block_length(
            &received_shards,
            self.n,
        );
        ...
        for i in 0..shard_len_in_syms {
            ...
            let piece = self
                .reconstruct_sub(&decoding_run[..], &erasures, &error_poly_in_log)
                .unwrap();
    ...
    pub fn encode_sub(&self, bytes: &[u8]) -> Result<Vec<Additive>> {
        assert!(
            is_power_of_2(self.n),
            "Algorithm only works for 2^i sizes for N"
        );
        assert!(
            is_power_of_2(self.k),
            "Algorithm only works for 2^i sizes for K"
        );
    ...
    pub fn reconstruct_sub(
        ...
        recovered
            .into_iter()
            .take(self.k)
            .for_each(|x| recovered_bytes.extend_from_slice(&x.0.to_be_bytes()[..]));
        Ok(recovered_bytes)
    }
